...
(function(){
 ...
    distance: (canvas.width/8).toFixed(1),
  ...
  function Color(min) {
    ...
    this.brighterStyle = createColorStyle(this.r+20, this.g+20, this.b+20);
  }
  ...
  function connectDots() {
    ...
    for(i = 0; i < dots.nb; i++){
      for(j = 0; j < dots.nb; j++){
        ...
        if(connectionCount > maxConnected)
              return;
      }
    }
  }
  ...
  var dotsMode = 1;
  var stepCounter = 0;
  var fps = 0, minFPS=10000, maxFPS=0, avgFPS = 0, now, lastUpdate = (new Date)*1, fpsFilter = 50;
...
*/
  ...
  function updateFPS() {    
    ...
    FPS.innerHTML = "FPS:" + fps.toFixed(2) + ", MAX:" + maxFPS.toFixed(2) + ", MIN:" + minFPS.toFixed(2) + ", MEAN:" + avgFPS.toFixed(2) + ", MC:" + maxConnected.toFixed(0);// + ", NB:" + dots.nb+ " ," + dotsMode;
    if(fpsCounter++ > 15) {      
      minFPS = fps;
      maxFPS = fps;
      fpsCounter = 0;      
